...
<title>Visualização de livros por categoria</title>
</head>
<body>
	<%@ page import="com.crudjspjava.dao.LivroDao, com.crudjspjava.bean.*, java.util.*"%>
	<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>

	<h1>Listagem de livros por categoria</h1>
	
	<%
		List<Livro> list = LivroDao.getAllbyCategoria(l);
		request.setAttribute("list", list);
	%>
	
	<table border="1">
		<tr><th>Código do livro</th><th>Titulo</th><th>Autor</th><th>Categoria</th><th>Valor</th></tr>
